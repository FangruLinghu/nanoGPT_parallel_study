...
log_files = ["output_dp_4g.log", "output_4g.log"]
labels = ["DDP 1NODE", "TP 1NODE"]
...
for log_file, label, color in zip(log_files, labels, colors):
    ...
    print(f"{label} total execution time: {total_time:.2f} seconds")
...
plt.ylim(bottom=0)
plt.tight_layout()
plt.savefig("loss_curve_compare1.png")
plt.show()
