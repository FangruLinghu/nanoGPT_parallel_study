...
global_param_count = sum(p.numel() for p in raw_model.parameters())
if (not ddp) or (ddp and ddp_rank == 0):
    print(f"[TP] Global param_count before TP: {global_param_count:,}")
...
# >>> LOGGER: wall-clock + stats init
wall_clock_start = time.time()
tokens_per_iter_theoretical = tokens_per_iter
total_time = 0.0
measured_iters = 0
time_warmup_iters = 10
gpu_util_sum = 0.0
gpu_util_count = 0
...
# decide parallel mode string
if use_tp and dp_size > 1:
    parallel_mode = "tp+fsdp"
elif use_tp and dp_size == 1:
    parallel_mode = "tp"
elif ddp:
    parallel_mode = "dp"
else:
...
logger = ParallelRunLogger(
    ...
    dp_size=ddp_world_size,
    param_count_override=global_param_count
)
# <<< LOGGER
...
while True:
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        with ctx:
            ...
            # check numeric
            if iter_num == 0 and micro_step == 0 and ddp and ddp_rank == 0:
    ...
    # >>> LOGGER: accumulate timing & GPU util
