...
def get_gpu_util():
    """return the avg of all gpu usages, fallback to -1.0"""
    try:
...
# >>> LOGGER: wall clock start & stats init will be just before training loop

# training loop
wall_clock_start = time.time()  # NEW: total runtime start
...
# stats for logger
tokens_per_iter_theoretical = tokens_per_iter
total_time = 0.0
measured_iters = 0
time_warmup_iters = 10  # skip first few iters when averaging timing
gpu_util_sum = 0.0
gpu_util_count = 0
...
# init ParallelRunLogger (model-related, parallel config)
parallel_mode = "dp" if ddp else "single"
...
while True:
    ...
    # >>> LOGGER: accumulate timing & GPU util (does not change training behavior)
    if iter_num >= time_warmup_iters:
        total_time += dt
        measured_iters += 1
...
# >>> LOGGER: summary logging with total runtime etc.
...
logger.log_summary(
    ...
    extra_fields={
        ...
        # DDP baseline doesn't have comm_monitor
        "avg_comm_time": None,
        "avg_comm_ratio": None,
        "avg_bandwidth_GBs": None,
