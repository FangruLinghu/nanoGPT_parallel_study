...
def get_gpu_util():
    """Return avg GPU utilization over all devices, -1.0 if unavailable."""
    try:
...
# >>> LOGGER: wall clock + stats init (just before training loop)
...
# compute parallel_mode & n_gpu for logger
if model_parallel > 1 and ddp_world_size > 1:
    parallel_mode = "dp+mp"
...
n_gpu_total = ddp_world_size * model_parallel
dp_size = ddp_world_size if ddp else 1  

param_dtype = next(raw_model.parameters()).dtype
param_dtype_str = str(param_dtype).replace("torch.", "")
run_id = f"{parallel_mode}_nlayer{n_layer}_emb{n_embd}_g{n_gpu_total}"
